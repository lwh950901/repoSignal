...
2. 生成报告：`python3 scripts/opportunity_analysis.py`
   ...
   - 新鲜度规则（对所有运行生效，含补跑历史日期）：同一方案最多连续出现两天；
     已连续出现两天的方案在第三天自动跳过，报告简介会注明。
     若固定模板拼不满 3 个方案，脚本会用今日新发现直接拼接补充方案
     （组件全部来自今日日报；业务名按组合能力面生成，与固定模板能力面一致时
     沿用模板名，因此同一业务方向跨天共享方案身份、一并参与新鲜度去重，
     不与已有方案重复）；当日拼不出可用组合时，报告允许只有 1-2 个方案。
