...
项目不仅有多语言入口，还公开了 200 份 PDF 的项目方实验、逐文档结果和基准工具；本周也有依赖升级与嵌套 PDF 拒绝服务风险修复，既能直接试用，也适合观察解析设计和安全响应。
...
项目已有稳定发布节奏，README、独立文档、路线图、贡献指南和采用者清单齐全；仓库还配置了 Playwright、CodeQL、Codecov 与 OpenSSF Scorecard 等质量和供应链检查。它的优势不是某个新功能，而是一套相对完整的工程闭环。
...
它的部署、迁移和集成入口较完整，既能直接作为应用使用，也适合阅读 Go 服务、数据库抽象、API 与前端之间的组织方式。本周报告记录的最新明确版本为 v0.30.0。

**注意：** 公开部署仍需自行配置访问控制、TLS、附件与数据库备份，并实际演练恢复。项目处于 v0.x，升级前要固定镜像版本、阅读迁移说明，不能把默认 Docker 参数当作生产安全配置。

### 4. RTK：在命令输出进入上下文前先做压缩

[RTK](https://github.com/rtk-ai/rtk) 是一个 Rust CLI 代理，在命令输出进入模型上下文前完成过滤、分组、截断和去重。它覆盖 `git`、搜索、测试、Docker 等 100 多个常见命令，并可通过 hooks 接入 Claude Code、Codex、Gemini CLI 和 Cursor 等工具。

**适合：** 经常让 Agent 读取终端输出，希望减少无效上下文，又不想先改造整套开发流程的个人或团队。

它把上下文工程落到一条可以直接测量的命令链路：在同一个仓库对比原始输出与压缩输出，就能观察信息保留、上下文体积和排障体验。8 月 8 日补录日报记录的最新可见 Release 为 v0.45.0。
...
项目把 Tree-sitter 解析、关系图、影响分析、评测和多种工具接入放在同一条链路中，并提供 tests、CI、SECURITY.md 与可复现评测材料。v2.3.7 还记录了多语言、MCP 并发和安全处理改进。
...
稳定 JSON 合约、退出码和 changed-file gate 让它便于自动化消费；仓库还包含 tests、fuzz、benchmarks、SECURITY.md 和 Docker 示例。本周发布的 v3.14.0 带验证签名。

**注意：** 框架约定、生成代码和缺失入口可能造成误报，type-aware 模式也不能替代 `tsc` 或完整测试。可选的 Fallow Runtime 属于付费能力，评估时要区分开源核心与商业层。

### 7. ECC：跨多个 Agent 宿主复用工程工作流

[ECC](https://github.com/affaan-m/ECC) 是一套面向 Claude Code、Codex、OpenCode、Kimi、Hermes 等宿主的 Agent harness、Skills、Hooks、记忆与安全工作流集合，可按需选择规则、命令和工具安装。

**适合：** 同时使用多个 Agent 客户端，希望复用工程规则、工具调用、测试、代码审查和记忆配置的开发者或团队。

v2.1.0 更新说明包含 Plan Canvas、跨宿主安装目标和统一 Memory Vault；仓库也提供安装文档、测试与 CI。它的看点不只是功能多，而是尝试把分散在不同客户端中的 Agent 工程习惯整理成可选择、可复用的资产。
...
[Microsandbox](https://github.com/superradcompany/microsandbox) 提供本地优先的 microVM 运行时、CLI 和 SDK，并支持 MCP/Skills 接入，可用于隔离 Agent、插件、CI 和自动化任务中的不可信代码，支持 Rust、Python、TypeScript 和 Go 调用。

**适合：** 需要为代码执行增加隔离层，或想拆解 microVM 生命周期与跨语言 SDK 设计的开发者。

Rust 核心、多个 SDK、示例、文档、安全材料和多平台发布资产形成了完整的学习路径；最新明确版本为 v0.6.8。它既能用最小示例观察隔离边界，也能沿调用链拆解 microVM 如何接入 Agent 工具。
...
项目将 Sensor、Detection 与 benchmark 分层组织，覆盖 300 多个任务、133 个 MCP server 和 17 类攻击技术；README 明确称 ADR 已在 Uber 生产部署，同时给出了可复现实验流程和 Sensor v1.0.0 Release。
...
`s01` 到 `s20` 的章节结构把叙事、代码、图示和测试分开，并提供中英日文说明及可执行入口，便于对照观察每一步新增的机制与复杂度。它的长处是一条可复现的学习路线，而不是包装成框架的示例代码。
