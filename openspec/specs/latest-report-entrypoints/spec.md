# latest-report-entrypoints Specification

## Purpose
定义月报、周报、日报和开源雷达周刊无日期入口的统一最新一期访问契约，通过精确的 Cloudflare 临时重定向将读者带到现有发现规则选出的最新带日期详情页，并约束构建期规则生成、输入验证、canonical 唯一性和失败行为。
