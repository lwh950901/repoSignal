...
- 追加 4（同日）：业务语义与方案身份改为单行 HTML 注释（读者不可见、脚本可解析）；解析侧恢复对注释后缀的兼容。生成器、测试、KUN-TASK 与定时任务提示词已同步。
- 追加 5（同日）：用户否决“隐藏”，改为彻底移除——新增 `feasibility/plan-history.jsonl`（`--rebuild-history` 从历史报告回填）作为跨日冷却的上游；报告不含任何机器元数据。生成器、测试、KUN-TASK 与定时任务提示词已同步。
- 验证：`python3 -m unittest scripts.test_opportunity_analysis` 78 项通过；
  `2026-09-21.md` 已用回归后的生成器重出（临时输出比对后落盘）。
- 追加 6（同日）：按用户反馈精简 `plan-history.jsonl`——去掉全空的业务三元组 id 字段（`plan_family` 已含三元组哈希，同一 family 必属同一三元组），track/证据状态无值时不再写出；冷却中段条件相应收窄为「新增关键组件或证据等级提升」（删除结构上无法命中的「客户问题变化」分支）；既有 58 条记录就地精简，全文件无空值。生成器、测试与 KUN-TASK 已同步（定时任务提示词无需变更）。
